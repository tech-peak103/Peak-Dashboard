...
function updateStats() {
    ...
    document.getElementById('activeSubjects').textContent = [...new Set(allSubjects)].length;
    document.getElementById('totalSubmissions').textContent = submissions.length;

    const totalRevenue = students.reduce((sum, s) => sum + (s.payment_amount || 0), 0);
    ...
    tbody.innerHTML = students.map(student => {
        ...
        return `
            <tr>
                ...
                <td><strong style="color:var(--primary-color);">${studentSubmissions.length}</strong></td>
                <td>${regDate}</td>
...
function loadSubmissionsTable() {
    ...
    if (submissions.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:2rem;">No submissions yet</td></tr>';
        return;
    }
...
function loadSubjectAnalytics() {
    ...
    const subjectStats = uniqueSubjects.map(subject => {
        ...
        return { subject, enrolledStudents, subjectSubmissions, completionRate };
    });
